...
func can_place(pos: Vector2i, rotation: int) -> bool:
	for cell in PIECE_SHAPES[current_piece][rotation]:
...
func lock_piece() -> void:
	for cell in PIECE_SHAPES[current_piece][current_rotation]:
...
func _draw() -> void:
	...
	for y in BOARD_HEIGHT:
		for x in BOARD_WIDTH:
			...
			if cell_id == EMPTY:
				draw_rect(Rect2(BOARD_OFFSET + Vector2(x * CELL_SIZE, y * CELL_SIZE), Vector2(CELL_SIZE, CELL_SIZE)), Color("#1b1b1b"), false, 1.0)
				continue
...
func draw_block(board_pos: Vector2i, color: Color) -> void:
	var pixel_pos := BOARD_OFFSET + Vector2(board_pos.x * CELL_SIZE, board_pos.y * CELL_SIZE)
